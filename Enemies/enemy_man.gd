...
@onready var sprite_enemy_man: Sprite2D = $Sprite2D
...
func _process(delta: float) -> void:
	if $AnimationEnemyMan.current_animation == "Attack":
		return
	...
	if not dead:
		...
		if player_in_area:
			if player.position.x - position.x < 0:
				...
				sprite_enemy_man.flip_h = true
				$AnimationEnemyMan.play("Walk")
			elif player.position.x - position.x > 0:
				get_node("AttackDetectionCollision").set_scale(Vector2(1,1))
				get_node("AttackCollision").set_scale(Vector2(1,1))
				position.x += (player.position.x - position.x) / speed
				sprite_enemy_man.flip_h = false
				$AnimationEnemyMan.play("Walk")
		else:
			sprite_enemy_man.flip_h = true
			$AnimationEnemyMan.play("Idle")
	if dead:
